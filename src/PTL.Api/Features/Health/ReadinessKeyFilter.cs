...
/// triggering) the DB-backed readiness check, not as an access-control
/// boundary. Note: an AWS ALB target-group health check cannot send custom
/// headers, so this only works for callers that can set one (a curl from an
/// ops box, an internal monitoring canary) - never point an ALB/ECS health
/// check directly at a key-gated route.
/// </summary>
