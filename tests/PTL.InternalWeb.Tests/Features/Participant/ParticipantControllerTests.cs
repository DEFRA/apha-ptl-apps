...
public class ParticipantControllerTests
{
    ...
    public async Task Details_ExistingParticipant_ReturnsViewWithModel()
    {
        ...
        Assert.Equal(participantId, model.Participant.ParticipantId);
    }
    ...
    [Fact]
    public async Task Create_Get_PopulatesCustomerContextAndAutoGeneratedLabCode()
    {
        var customerId = Guid.NewGuid();
        var controller = CreateController(
            new FakeParticipantApiClient(),
        ...
        Assert.Equal("Sample Customer", model.CustomerName);
        Assert.Equal("QAL0001", model.CustomerQalNumber);
        Assert.False(string.IsNullOrWhiteSpace(model.LabCode));
        Assert.True(int.TryParse(model.LabCode, out var labCode) && labCode >= 1000);
    }
    ...
    [Fact]
    public async Task Edit_Post_ValidationError_ReturnsViewWithError()
    {
        var participantId = Guid.NewGuid();
        var customerId = Guid.NewGuid();
        var apiClient = new FakeParticipantApiClient
        ...
        var model = new PTL.InternalWeb.Features.Participant.ParticipantFormViewModel
        {
            ...
            CustomerId = customerId,
            LabCode = "LAB-01",
            LabName = string.Empty  // Empty to trigger validation error
        };
        ...
        Assert.Same(model, view.Model);
        Assert.False(controller.ModelState.IsValid);
        Assert.True(controller.ModelState.ContainsKey("LabName"));
    }
}
